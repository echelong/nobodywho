...
pub enum ItemKind {
    Reasoning {
        // An absent part list and an empty one mean the same thing.
        #[serde(default)]
...
impl Item {
    pub fn init_function_call(item_id: ItemId, call_id: FunctionCallId) -> Self {
        Item {
            id: item_id,
...
impl ResponseObject {
    ...
    fn get_item(&self, output_index: OutputIndex) -> Option<&Item> {
        self.output
            .get(usize::try_from(output_index.0).expect("OutputIndex must be a valid usize"))
    }

    fn get_item_mut(&mut self, output_index: OutputIndex) -> Option<&mut Item> {
        self.output
            .get_mut(usize::try_from(output_index.0).expect("OutputIndex must be a valid usize"))
    }
    ...
    pub fn consume_event(&mut self, event: StreamEvent) -> Result<(), EventStreamError> {
        ...
        match kind {
            ...
            EventKind::ContentPartAdded(ContentPartAddedEvent {
                ...
                match item.kind {
                    ItemKind::Reasoning { ref mut content, .. } | ItemKind::Message { role: _, ref mut content } => {
                    ...
                    ItemKind::FunctionCall {
                        ...
                        name: _,
                        arguments: _,
                    } | ItemKind::FunctionCallOutput { call_id: _, output: _ } => panic!(
            ...
            EventKind::ContentPartDone(ContentPartDoneEvent {
                ...
                match item.kind {
                    ItemKind::Reasoning { ref content, .. } | ItemKind::Message { role: _, ref content } => {
            ...
            EventKind::ReasoningTextDelta(ReasoningTextDeltaEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningTextDelta");
            ...
            EventKind::ReasoningTextDone(ReasoningTextDoneEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningTextDone");
                let kind = item.kind.item_type();
                match item.kind {
                    ItemKind::Reasoning { ref content, .. } => {
                        let part = content.get(content_index.0 as usize).expect(
            ...
            EventKind::ReasoningSummaryPartAdded(ReasoningSummaryPartAddedEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningSummaryPartAdded");
                let kind = item.kind.item_type();
                match item.kind {
                    ItemKind::Reasoning {
                        ref mut summary, ..
            ...
            EventKind::ReasoningSummaryPartDone(ReasoningSummaryPartDoneEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningSummaryPartDone");
                let kind = item.kind.item_type();
                match item.kind {
                    ItemKind::Reasoning { ref summary, .. } => {
            ...
            EventKind::ReasoningSummaryTextDelta(ReasoningSummaryTextDeltaEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningSummaryTextDelta");
                let kind = item.kind.item_type();
                match item.kind {
                    ItemKind::Reasoning {
                        ref mut summary, ..
            ...
            EventKind::ReasoningSummaryTextDone(ReasoningSummaryTextDoneEvent {
                ...
                let item = self.item_for_mut(output_index, &item_id, "ReasoningSummaryTextDone");
                let kind = item.kind.item_type();
                match item.kind {
                    ItemKind::Reasoning { ref summary, .. } => {
                        let part = summary.get(summary_index.0 as usize).expect(
            ...
            EventKind::FunctionCallArgumentsDelta(FunctionCallArgumentsDeltaEvent {
                ...
                match item.kind {
                    ItemKind::FunctionCall {
                        ...
                        name: _,
                        ref mut arguments,
                    } => {arguments.push_str(&delta); Ok(())},
                    ItemKind::Reasoning { .. }
                    | ItemKind::FunctionCallOutput { call_id: _, output: _ }
                    | ItemKind::Message { role: _, content: _ } => panic!(
